...
class ProcessorTest extends \PHPUnit\Framework\TestCase
{
    ...
    public function testComplexityReducer()
    {
        ...
        foreach ([
            'errors' => [
                [
                    'message' => 'Field "badfield" not found in type "User". Available fields are: "firstName", "lastName", "code", "likes"',
                    'locations' => [['line'=>1, 'column'=>8]]
                ]
            ]
        ] as $key => $value) {
            $this->assertArrayHasKey($key, $processor->getResponseData());
            $this->assertSame($value, $processor->getResponseData()[$key]);
        }
